...
SECONDS_PER_ROLE = 60   # <-- change this single number to speed up/slow down rotation
...
for idx, role in enumerate(ROLES):
    start = idx * SECONDS_PER_ROLE
    end = start + SECONDS_PER_ROLE

    key_times = [0, start / total_duration, end / total_duration, 1]
    key_values = [0, 1, 0, 0]
    if start == 0:
        key_times = [0, end / total_duration, 1]
        key_values = [1, 0, 0]

    kt_str = ";".join(f"{t:.5f}" for t in key_times)
    kv_str = ";".join(str(v) for v in key_values)

    svg_parts.append(f'''
  <g opacity="0">
    <animate attributeName="opacity" values="{kv_str}" keyTimes="{kt_str}" calcMode="discrete" dur="{total_duration}s" begin="0s" repeatCount="indefinite"/>
...
print(f"Generated continuously-rotating role card ({n} roles, {SECONDS_PER_ROLE}s each) -> {OUTPUT_PATH}")
